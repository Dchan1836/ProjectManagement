...
export default function Dashboard() {
  ...
  const priorityCounts = tasks?.reduce(
    (acc, task) => {
      acc[task.priority || "Normal"] =
        (acc[task.priority || "Normal"] || 0) + 1;
      return acc;
    },
    {} as Record<string, number>,
  );
      ...
      <div className="space-y-6 h-full flex flex-col">
        ...
        <div className="flex-1 min-h-0">
          <DashboardLayoutComponent
            ...
            resizeStop={onResizeStop.bind(this)}
            cellAspectRatio={1.2}
            allowDragging={true}
            allowFloating={true}
                ...
                <div className="p-4 h-[calc(100%-48px)] overflow-auto">
                  <div className="space-y-3">
                    {tasks?.slice(0, 5).map((task) => (
                      <div
                        key={task.id}
                        className="bg-secondary/30 p-3 rounded-xl border border-border/50 hover:border-primary/30 transition-colors"
                      >
