  ...
  return (
    <DialogComponent
      ...
      buttons={[
        ...
        { buttonModel: { content: "Cancel" }, click: onClose },
      ]}
...
//    args.cancel = true; // Prevents default Syncfusion dialog
  ...
  const contextMenuOpen = (args) => {
    ...
    if (generalTab) {
      ...
      generalTab.childNodes.forEach((node) => {
        // Node.TEXT_NODE (3) represents a text node
        if (node.nodeType === Node.TEXT_NODE) {
          // Trim to remove excess whitespace from the HTML structure
          const trimmedText = node.textContent.trim();
          if (trimmedText.length > 0 && !trimmedText.includes("..")) {
            node.textContent += "...";
          }
        }
      });
      ...
      <div className="bg-card border border-border rounded-xl p-4 shadow-sm">
        <div className="flex flex-wrap gap-4 items-center">
          ...
          <Select value={statusFilter} onValueChange={setStatusFilter}>
            <SelectTrigger
              className="w-[150px]"
              data-testid="select-status-filter-gantt"
            >
          ...
          <Select value={priorityFilter} onValueChange={setPriorityFilter}>
            <SelectTrigger
              className="w-[150px]"
              data-testid="select-priority-filter-gantt"
            >
          ...
          <Select value={assigneeFilter} onValueChange={setAssigneeFilter}>
            <SelectTrigger
              className="w-[150px]"
              data-testid="select-assignee-filter-gantt"
            >
        ...
        <ButtonComponent onClick={openAddDialog}>
          Open Add Dialog
        </ButtonComponent>
      </div>
